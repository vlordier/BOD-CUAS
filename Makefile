...
clean: ## Kill all demo processes and clean state
	# Kill by binary name with path prefix for safety
	-pkill -f "target/release/counter-uas-director" 2>/dev/null || true
	-pkill -f "target/release/furia-core-server" 2>/dev/null || true
	-pkill -f "target/release/dev-atak-server" 2>/dev/null || true
	-pkill -f "target/release/s1-sim-server" 2>/dev/null || true
	-pkill -f "target/release/cuas-health-injector" 2>/dev/null || true
	-pkill -f "target/release/sapient-simulator" 2>/dev/null || true
