...
fail=0
check_dir() { local name="$1" path="$2"; if [[ -d "$path" ]]; then printf '%-28s ✅ %s\n' "$name" "$path"; else printf '%-28s ❌ %s\n' "$name" "$path"; fail=1; fi; }
check_file() { local name="$1" path="$2"; if [[ -f "$path" ]]; then printf '%-28s ✅ %s\n' "$name" "$path"; else printf '%-28s ❌ %s\n' "$name" "$path"; fail=1; fi; }
check_cmd() { local name="$1" cmd="$2"; if command -v "$cmd" >/dev/null 2>&1; then printf '%-28s ✅ %s\n' "$name" "$cmd"; else printf '%-28s ❌ %s\n' "$name" "$cmd"; fail=1; fi; }
check_port_free() {
  local port="$1" label="$2"
  if python3 - "$port" <<'PY'
import socket, sys
port = int(sys.argv[1]); s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
check_git_repo() {
  ...
  if git -C "$path" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    branch="$(git -C "$path" branch --show-current 2>/dev/null || true)"; sha="$(git -C "$path" rev-parse --short HEAD 2>/dev/null || true)"
    [[ -n "$(git -C "$path" status --porcelain 2>/dev/null || true)" ]] && dirty=" DIRTY"
    printf '%-28s ✅ %s@%s%s\n' "$name" "${branch:-detached}" "$sha" "$dirty"
  else printf '%-28s ❌ not a git checkout: %s\n' "$name" "$path"; fail=1; fi
}

printf 'Furia Bordeaux golden demo doctor\n\n'
check_dir 'BOD-CUAS' "$ROOT/BOD-CUAS"; check_dir 'furia-core' "$ROOT/furia-core"; check_dir 'furia-ui' "$ROOT/furia-ui"; check_dir 'furia-c2' "$ROOT/furia-c2"; check_dir 'S1' "$ROOT/S1"
printf '\nRequired demo files\n'
...
printf '\nToolchain\n'
check_cmd 'git' git; check_cmd 'cargo' cargo; check_cmd 'node' node; check_cmd 'pnpm' pnpm; check_cmd 'python3' python3; check_cmd 'curl' curl
...
if command -v git >/dev/null 2>&1; then
  printf '\nRepository revisions\n'; check_git_repo 'BOD-CUAS revision' "$ROOT/BOD-CUAS"; check_git_repo 'furia-core revision' "$ROOT/furia-core"; check_git_repo 'furia-ui revision' "$ROOT/furia-ui"; check_git_repo 'furia-c2 revision' "$ROOT/furia-c2"; check_git_repo 'S1 revision' "$ROOT/S1"
fi
if command -v python3 >/dev/null 2>&1; then
  printf '\nRequired ports\n'; check_port_free 4222 'NATS TCP'; check_port_free 9222 'NATS WebSocket'; check_port_free 8080 'ATAK dev server'; check_port_free 3000 'Furia Core'; check_port_free 3227 'S1 sim server'; check_port_free 5173 'Furia C2'
fi
