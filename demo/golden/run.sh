...
echo '=== Arm canonical closed-loop acceptance monitor ==='
NATS_URL="$NATS_URL" DEMO_SPEED="$DEMO_SPEED" python3 "$SCRIPT_DIR/verify_canonical.py" >"$LOG_DIR/verify.log" 2>&1 &
VERIFY_PID=$!
PIDS+=("$VERIFY_PID")
sleep 0.5
...
# Do not declare the demo ready until the complete authority -> delegation ->
# execution-evidence -> safety-policy -> abort -> SafeHold loop is observed.
if ! wait "$VERIFY_PID"; then
  echo 'ERROR: Bordeaux canonical golden demo acceptance failed.' >&2
  cat "$LOG_DIR/verify.log" >&2 || true
  exit 1
fi
printf '%-28s ✅\n' 'Canonical closed-loop acceptance'
...
Acceptance proved: named operator authorization -> Core mission delegation -> S1 accepted/active evidence -> Core safety abort -> S1 abort result -> Aborted/SafeHold FSM.
